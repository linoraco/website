...
const FormAddSuratOlahraga = () => {
  ...
  const loadImage = async (e) => {
    const gambar_surat_olahraga = e.target.files[0];
    setFile(gambar_surat_olahraga);
    setPreview(URL.createObjectURL(gambar_surat_olahraga));
  };
  ...
  const saveGambar = async (e) => {
    ...
    formData.append("nama_surat_olahraga", title);
    formData.append("tanggal_surat_olahraga", tanggal);

    try {
      await axios.post("http://localhost:4000/suratolahraga", formData, {
        headers: {
      ...
      setOpenModal(true);
      navigate("/surat-kerja-olahraga-kadis");
    } catch (error) {
...
export default FormAddSuratOlahraga;
